...
First, we need to configure VS Code to be useful for you.  Open your settings by chooing the File menu.  Then click Preferences, then choose Settings.

 In the search bar on the settings page, search for "inline suggest".  You'll have to look down the list, but find "**Editor – Inline Suggest: Enabled**" and *uncheck* the box (to disable it).
...
Change into the a1.2-AI-Exploration folder:

```
cd a1.2-AI-Exploration
```
...
`HelloClass.java` there.  Double click it to to open the file.
...
**Save your file!** The compiler will compile what is written to disk,
  ...
  Terminal again if it was closed and move back to a1.2-AI-Exploration.

Verify you're in the right folder by using `ls` (lists the contents of the folder). Note that the first character is a lower case L:

```
ls
```

You should see `HelloClass.java`.  If you don't, change into the a1.2-AI-Exploration folder.
